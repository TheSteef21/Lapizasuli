...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sovereign Hub | SADV41</title>
    ...
    html_content += f"""
        ...
        <section id="monitor-sismico-seccion" class="max-w-3xl mx-auto p-6 bg-[#0d1117] border border-slate-800 rounded-2xl shadow-xl">
            <div class="text-center py-4">
                ...
                <p class="text-slate-400 text-xs">Conexión Directa con el Servidor Operacional de Render</p>
            </div>
    ...
    <script>
        ...
        async function refrescarMonitor() {{
            try {{
                ...
                let htmlContenido = `<div class="glass-sismico p-5 rounded-xl"><p class="text-xs text-slate-300 font-mono">\${{data.analisis_ia}}</p></div>`;
                data.eventos.forEach(sismo => {{
                    htmlContenido += `
                        <div class="p-4 rounded-xl bg-slate-950/50 border border-slate-800 border-l-4 border-l-red-500">
                            <span class="text-xs block text-white font-bold">📍 \${{sismo.ubicacion}}</span>
                            <span class="text-xs font-mono text-rose-400">Magnitud: M \${{sismo.magnitud}} | Profundidad: \${{sismo.profundidad_km}} km</span>
                        </div>`;
                }});
                document.getElementById("monitor-salida").innerHTML = htmlContenido;
            }} catch (error) {{
                document.getElementById("monitor-salida").innerHTML = "<p class='text-xs text-red-400'>Error de sincronización con la API Central.</p>";
            }}
        }}
